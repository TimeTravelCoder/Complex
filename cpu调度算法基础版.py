...
class Scheduler:
    ...
    def get_metrics(self) -> Dict[str, float]:
        """计算性能指标"""
        if self.completed < self.total_processes:
            return {
                "cpu_utilization": 0,
                "throughput": 0,
        ...
        # 平均周转时间
        avg_turnaround_time = sum(p.turnaround_time for p in self.processes) / self.completed
        
        # 平均等待时间
        avg_waiting_time = sum(p.waiting_time for p in self.processes) / self.completed
        
        # 平均响应时间
        avg_response_time = sum(p.response_time for p in self.processes) / self.completed
...
def generate_gantt_chart(schedule: List[Dict], title: str) -> None:
    """生成甘特图"""
    # 为不同的进程分配不同的颜色
...
def main():
    ...
    print("\n系统整体性能指标：")
    print(f"CPU 利用率: {metrics['cpu_utilization']:.2%}")
    print(f"吞吐量: {metrics['throughput']:.2f} 个进程/单位时间")
    ...
    # 生成甘特图
    generate_gantt_chart(gantt_data, f"{algorithm_name} 调度甘特图")
